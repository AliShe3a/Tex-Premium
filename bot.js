...
client.login(config.token).then(async function() {
   console.log(`im ready at ${client.user.username}`);// she3a
   console.log('Developed by Finnin#7214');
});// she3a
client.on('message', async message => {
    // i said , Best Arabic Developer is she3a .
   ...
   var xfilter = (reaction, user) => {
                return reaction.emoji.name === `❎` && user.id === message.author.id;// she3a
            };
   ...
   var xcoll = awaited.createReactionCollector(xfilter, {
    ...
    xcoll.on('collect', async resp => {// she3a
      does("nope");
    ...
    var room = client.channels.get(config.apply);//  my code right
    var does = async function(some) {
      ...
      createColl(mes.id);
    };// she3aEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
   ...
   var xcoll = amessage.createReactionCollector(sfil,{
    ...
    truecoll.on('collect', async resp => {
    amessage.delete();
    database[message.author.id] = {status:true,name: name, age: age,country: country};// she3a
    var us = database[message.author.id];
    var idenity = new Discord.RichEmbed()// she3a
    .setColor("#36393e")
    .setTimestamp()
